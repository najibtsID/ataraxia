  ...
  return (
    <section className="products container reveal-on-load reveal-on-scroll">
      <div className="slider-controls">
        ...
        <div>
          <button className="btn ghost" onClick={prev} aria-label="prev">‹</button>
          <button className="btn ghost" onClick={next} aria-label="next">›</button>
        </div>
      </div>
      {loading ? <p>Memuat produk...</p> : (
        <div ref={ref} className={`product-grid scroller ${inView ? 'in-view' : ''}`}>
          <div ref={scrollerRef} className="product-scroller">
            {items.map((p, idx) => (
              <article key={p.id} className={`product-card ${p.featured ? 'featured' : ''}`}>
                ...
                <div className="product-body">
                  <h3>{p.name} <span className="tag">{p.category}</span></h3>
                  <p className="desc">
                    {typeof p.description === 'string' ? p.description : documentToReactComponents(p.description)}
                  </p> 
                  <div className="meta"></div>
                  <button className="btn primary no-border" onClick={()=> setOpenProduct(p)} aria-haspopup="dialog">Info Lengkap</button> 
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="slider-dots">
            {items.map((_,i)=> (
              <button key={i} className={i===active ? 'active' : ''} onClick={()=>goTo(i)} aria-label={`to ${i+1}`}></button>
            ))}
          </div>
        </div>
      )}
      ...
      {openProduct && (
        <ProductModal product={openProduct} onClose={()=> setOpenProduct(null)} waNumber={waNumber} />
      )}
    </section>
  );
}
